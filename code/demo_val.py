...
model_folder = f"/app/duy55/segmentation/code/demo/{model_name}"
...
model.to(device)
with torch.no_grad():
    for image_num, (val_images, val_labels) in tqdm(enumerate(testloader)):
        ...
        for i in range(val_images.size(0)):
            ...
            decoded_gt = test_data.decode_segmap(ground_truth[i])
            
            combined_image = np.hstack((decoded_pred, decoded_gt))
            
            combined_samples.append(combined_image)
            saved_samples += 1
...
for i, combined_image in enumerate(combined_samples):
    ...
    plt.axis('off')
    save_path = os.path.join(model_folder, f"combined_sample_{i}.png")
    plt.savefig(save_path, bbox_inches='tight', pad_inches=0)
    plt.clf()

print("Saved combined samples successfully!")